...
class CurrentLocationViewController: UIViewController, CLLocationManagerDelegate, CAAnimationDelegate {
        ...
        if let location = location {
            ...
            if let error = lastLocationError as NSError? {
                if error.domain == kCLErrorDomain && error.code == CLError.denied.rawValue {
                    statusMessage = "Location Services disabled for this app."
                } else {
                    statusMessage = "Error getting location."
                }
            } else if !CLLocationManager.locationServicesEnabled() {
                statusMessage = "Location Services disabled for this device."
    ...
    func configureGetButton() {
        ...
        if updatingLocation {
            ...
            if view.viewWithTag(spinnerTag) == nil {
                ...
                spinner.center.y += spinner.bounds.size.width / 2 + 25
    ...
    func animationDidStop(_ anim: CAAnimation, finished flag: Bool) {
        ...
        containerView.center.x = view.bounds.size.width
        containerView.center.y = 40 + containerView.bounds.size.height / 2
    ...
    func loadSoundEffect(_ name: String) {
        if let path = Bundle.main.path(forResource: name, ofType: nil) {
            ...
            if error != kAudioServicesNoError {
                print("Error code: \(error) loading sound at \(path)")
            }
        }
    }
